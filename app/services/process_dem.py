...
hs = (np.sin(alt_rad) * np.cos(np.radians(slope)) + np.cos(alt_rad) * np.sin(np.radians(slope)) * np.cos(az_rad - np.radians(aspect)))
...
for name, date in [("slope", slope), ("aspect", aspect), ("hillshade", hillshade), ("curvature", curvature)]:
